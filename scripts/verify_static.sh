...
grep -q '전체 플레이리스트를 순서대로 추출' "$root/app/src/main/java/com/ytet/android/ui/MainActivity.java"
grep -q 'MusicBrainz로 실제 앨범/아티스트 보정' "$root/app/src/main/java/com/ytet/android/ui/MainActivity.java"
